...
mirrors the same tokens for the parts Qt stylesheets *can't* reach â€” the
...
vocabulary â€” a screen that reaches outside them is what makes an app stop
...
a retired token is aliased onto its replacement rather than deleted â€” see
...
# â”€â”€ tonal ramps (one shared lightness scale, so step N of any role matches) â”€â”€
...
# â”€â”€ core roles â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
TEXT = "#1d1f20"
ACCENT = "#4480bb"
ACCENT_2 = "#628fbb"

# â”€â”€ the six surfaces, one value each â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
# The census found seventeen distinct greys shipping in the stylesheet with no
# rule about which meant what â€” three near-identical hairlines, two wells, two
...
# rotating with the role like everything else â€” a manager who switches profile
...
CANVAS = "#f4f5f6"             # page background â€” the workspace behind cards
...
# BG was #f2f2f3 and SURFACE #e9e9ea â€” a second page grey two units off CANVAS
# and a second border grey two units off NEUTRAL[200]. Neither difference was
# visible; both were reachable by accident.
BG = CANVAS
SURFACE = NEUTRAL[200]
CARD_LINE = HAIRLINE           # was #f0f0f1 â€” a third hairline, now the one
NEUTRAL_350 = "#c2c2c5"        # keyboard hints, inactive step numerals

# â”€â”€ semantic roles â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# register stops being readable at a glance â€” which is the one thing it is for.
#
...
# OK_INK #1a7a5e at 4.88 â€” both close enough to the line that any future nudge
...
WARN_BG = "#fff8e8"
ERR = "#8a2f2f"
ERR_INK = ERR                  # 7.37:1 on ERR_BG â€” named for symmetry
ERR_BG = "#fdeeee"
ERR_LINE = "#eecccc"
...
# they are already in _ACCENT_HEXES below and already rotate â€” no new hex has
...
# rules in style.qss and the two disagreeing for half the tones â€” a "warning"
...
# â”€â”€ execution states â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
#                     has stop() and nothing else â€” a one-way threading.Event.
...
# already exists â€” _smart_wait polls the response length every five seconds
# and holds the character count â€” it is simply never emitted. It is defined
...
STATUS = {
    ...
    # and the canvas is #f4f5f6 â€” one unit apart, so a "Queued" badge sitting on
    ...
    # Waiting is neutral and dashed, NOT accent â€” the audit's tenth defect is
...
def status_key(name: str) -> str:
    """The canonical STATUS key for a state name â€” see status()."""
    key = (name or "").strip().lower().replace("-", "_").replace(" ", "_")
    key = _STATUS_ALIASES.get(key, key)
    return key if key in STATUS else "idle"


# â”€â”€ elevation â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# reserved for things that genuinely float â€” modals, dropdowns, a dragged row.
...
# â”€â”€ radii â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
# Three values, not six. The census counted thirteen distinct radii painting
# against six declared tokens â€” R_MODAL was used nowhere at all, R_CHIP had
...
R_HERO = R_CARD                 # was 14 â€” a hero card is a card
R_MODAL = R_CARD                # was 16 â€” a modal is a card
...
# â”€â”€ spacing â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
# A 4px base, seven steps, nothing between them. The census found 37 distinct
# content-margin tuples and 20 distinct layout spacings across the panels â€”
...
PAGE_PAD = SPACE_6              # 28 â€” page edge to content, every screen
CARD_PAD = SPACE_5              # 20 â€” card edge to content
CARD_GAP = SPACE_4              # 16 â€” gutter between cards in a grid
ROW_GAP = SPACE_3               # 12 â€” between rows inside one card

# â”€â”€ type â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
FONT_BODY = "Barlow"
FONT_HEADING = "Barlow Condensed"
# No mono face is vendored â€” it is wanted for ids, keys, device codes and
...
# single 4px band â€” 11, 11.5, 12, 12.5, 13, 13.5, 14, 14.5, 15 â€” almost all
...
# same metric â€” see font() below, which sets a pixel size on the QFont for
...
# old names working â€” six heading levels collapse onto three real sizes â€” so
...
def font(level: str = "BODY", weight: int = 0) -> QFont:
    ...
    beside it â€” which is the whole failure this module exists to prevent.
    """
...
def type_css(level: str = "BODY", colour: str = "") -> str:
    ...
    stylesheet. Always use this rather than retyping a size â€” a bare
...
def load_fonts() -> None:
    ...
    Barlow / Barlow Condensed pairing â€” without it Qt silently falls back to a
...
# â”€â”€ per-role accent â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
# A company running Prism has one copy per person, and the fastest way to know
# whose copy you are looking at â€” or which profile a manager has switched into
# â€” is that the whole app is a different colour.
#
...
# any hex that must NOT rotate â€” every neutral, every semantic, every tool
# brand colour â€” has to be distinct from all eleven. The INFO role added above
...
def apply_role(hue: int) -> None:
    ...
    or the seams show â€” which is the same reason this module exists at all.
...
def role_stylesheet(qss: str, hue: int) -> str:
    ...
    eleven hexes are only ever used as the accent â€” the neutrals, the canvas
...
# â”€â”€ helpers for painted widgets â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
def c(hex_or_role: str, alpha: float = 1.0) -> QColor:
    """QColor from a token, optionally at partial alpha (the QSS equivalent of
    color-mix(â€¦ N%, transparent))."""
...
# Perplexity and the deck went to Gamma â€” which a row of near-identical blues
...
# later never renders un-styled â€” and never collides with a brand colour.
_BADGE_CYCLE = [ACCENT_RAMP[800], NEUTRAL[700], ACCENT_RAMP[600],
                ACCENT_RAMP[700], NEUTRAL[800], "#486077"]
...
def over(alpha: float, fg: str = "#ffffff", bg: str = None) -> str:
    ...
    QSvgRenderer, and rgba() there is not dependable across Qt's SVG backends â€”
