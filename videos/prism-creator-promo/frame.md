...
colors:
  ...
  ink-on-orange-faint: "rgba(11,12,14,0.40)"
  ink-on-orange-border: "rgba(11,12,14,0.20)"
  # — spectrum: the documented brand exception (see § Colors). One band per
  #   Prism category, in plan order. Meaning-bearing, never decorative.
  band-research:  "#A78BFA"
  band-reasoning: "#60A5FA"
  band-writing:   "#4CD9B4"
  band-images:    "#A3E635"
  band-video:     "#FACC15"
  band-build:     "#FB923C"
...
components:
  ...
  spectrum-band:
    layout: "full-bleed horizontal band, 0 radius, flat fill from the band-* ramp"
    label: "{typography.label} in {colors.ink-black} ON the band, or in the band hue on ink"
    rule: "A band NEVER carries meaning by hue alone — every band is labelled with its stage name."
    description: "The refraction primitive. One band per Prism category, always in plan order (research → reasoning → writing → images → video → build → decks). Bands enter by wiping outward from the white prompt line, never by fading in as a set."
  tool-chip:
    layout: "sharp rectangle, 1px solid {colors.border-dark}, ink-black-alt fill"
    typography: "{typography.label} in {colors.cream}, with a 2px left edge in the owning band hue"
    description: "The real tool the stage runs in — Perplexity / ChatGPT / Claude / Claude Design / Kimi 2.6. Names are real; never invent a tool."
  ui-inset:
    layout: "cropped region of a real app screenshot, 1px {colors.border-dark} frame, 0 radius, no shadow"
    rule: "Crop to the region that carries the beat. NEVER shrink a whole desktop window into the vertical frame."
    description: "Real product evidence staged as a lit panel on the ink ground."
---
...
- **Fonts** — already set to **Barlow Condensed** (display) / **Barlow** (body); ignore any preset font name lingering in prose.
- **Colors** — use the frontmatter hex; preset color NAMES in prose (e.g. "cobalt", "cream") mean the remapped brand values. **`fire-orange` now means Prism teal `#4CD9B4`** — the logo's accent. Read every "orange" in the prose below as teal.
- **The one documented exception to the preset:** broadside says *one accent, never a second hue.* Prism's brand says the opposite and says it first — "Colour = meaning: each of the 7 categories owns a band of the spectrum" (PRODUCT.md). The `band-*` ramp exists for that and **only** for that. It is not a licence for decorative colour: a hue may appear only when it names a Prism category, only on the `spectrum-band` / `tool-chip` components, and never without its label. Everywhere else the one-accent rule holds exactly as written — teal is still the lone accent on every frame that is not showing the refraction.
