...
class TheArtDirectorIsToldWhatGoodMotionIs(unittest.TestCase):
    """The prompt covered the MECHANICS of motion — fill-mode, seeking, which
    custom property holds what — and never said what motion should DO. Scenes
    came back authored in isolation, each animating in and out on its own, so
    the eye's momentum died at every cut and the reel read as a stack of
    slides.

    Laws adapted from HyperFrames' motion doctrine (Apache 2.0).
    """

    def setUp(self):
        import core_bridge  # noqa: F401
        from core import reel_web
        self.text = reel_web.design_instructions()

    def test_the_seam_rule_is_there(self):
        """The commonest violation: a scene exits left and the next enters
        from the right, which reads as a mistake because it is one."""
        low = self.text.lower()
        self.assertIn("how a scene leaves decides how the next one arrives", low)
        self.assertIn("same axis, same direction", low)

    def test_one_dominant_direction_is_named(self):
        self.assertIn("PICK ONE CURRENT", self.text)
        self.assertIn("opposing directions", self.text)

    def test_cuts_land_mid_motion(self):
        self.assertIn("CUT MID-MOTION", self.text)
        self.assertIn("dead beat", self.text)

    def test_idle_wobble_is_banned(self):
        """The single clearest tell that a reel was generated rather than
        designed: everything gently floating to fill time."""
        low = self.text.lower()
        self.assertIn("motion performs, then stops", low)
        for word in ("drifts", "breathes", "floats"):
            self.assertIn(word, low)

    def test_stillness_before_the_climax(self):
        self.assertIn("STILL BEFORE THE CLIMAX", self.text)

    def test_it_speaks_prisms_contract_not_hyperframes(self):
        """Adapted, not copied. Theirs assumes GSAP, data-* timing and a
        ledger.json verifier — none of which exist here, and instructing a
        model to use machinery that is not present is worse than silence."""
        low = self.text.lower()
        for foreign in ("gsap", "ledger.json", "data-frame", "hyperframes"):
            self.assertNotIn(foreign, low, foreign)

    def test_the_mechanics_section_still_stands(self):
        """The new block is in ADDITION to the technical rules, not instead of
        them. A beautiful seam that does not survive seeking is not filmed."""
        self.assertIn("HOW MOTION WORKS", self.text)
        self.assertIn("both", self.text)

    def test_the_deferred_work_is_written_down(self):
        """Sizes 2 and 3 were consciously not built. An intention that lives
        only in a chat log is an intention that is lost."""
        here = os.path.dirname(os.path.abspath(__file__))
        path = os.path.join(here, "..", "docs", "DEFERRED.md")
        self.assertTrue(os.path.exists(path), "docs/DEFERRED.md is missing")
        body = open(path, encoding="utf-8").read()
        self.assertIn("Trigger:", body, "an entry with no trigger is a wish")
        self.assertIn("carriers across the cut", body.lower())
