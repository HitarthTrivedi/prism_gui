...
# Priced BOQ — the take-off becomes a tender-ready Excel, priced offline

The BOQ add-on measured a drawing and then handed the numbers to a browser AI to
"format" — which left Rate and Amount blank and trapped the result in a browser
tab. A Vadodara contractor's actual deliverable is a *priced* BOQ in Excel, so
that was the missing half.

**Was:** `core.boq` measured quantities → an auditable CSV → a browser LLM wrote
them up with blank rates. The only local file was the raw quantities CSV; the
finished document lived in a Claude/ChatGPT tab (`_links["format"]`).

**Now:** a new deterministic pricing layer, `core.boq_price`, turns measured
quantities into a priced `Boq` (item → unit → qty → rate → amount, grouped into
sections) and exports a tender-ready `.xlsx` with LIVE formulas — `=Qty*Rate`
per line, `=SUM` sub-totals, contingency, works-contract GST (CGST+SGST intra-
state, IGST inter-state), a grand total and the amount in Indian words — plus a
linked **Abstract of Cost** sheet. Rates come from a library the user brings
(their price list or a DSR/SOR export, read by `quoting.load_rates`) or types;
`quoting.match_item` suggests them. `BoqDialog` gained an editable pricing grid
(measured quantities locked, rates editable, amounts live) with **Load rate
list**, contingency/GST controls and an **Export priced Excel BOQ** button, shown
once a drawing is measured. Reached through `core_bridge.get_boq_price()`.

**The rule that shapes it:** no number of record is ever generated by an AI.
Quantities are the measured geometry; rates are library or user values; every
amount/tax/total is arithmetic (Decimal here, a live formula in the sheet). A
library rate is auto-applied ONLY when its unit matches the measured quantity —
a wall measured in metres is never priced at a per-cum rate; the mismatch is
flagged and the line left unpriced, because a wrong rate on a tender is worse
than an obvious gap. Unpriced lines are shaded red and counted, never shipped
silently.

`tests/test_boq_pricing.py` (14) pins the arithmetic, the unit guard, the Indian
words and the live-formula export. `core.boq_price` carries no GUI/licence
dependency, so the CLI (`cmd_boq`) can gain the same export next; a government-
format Schedule A+B PDF (rendered from the same model via the bundled browser
engine) is the planned fast-follow.
