...
class EmailPanel(_AddonFrontDoor):
    ...
    STEPS = [
        ("user", "Say who it is for",
         ...
         "goal."),
        ("pencil", "Prism drafts it",
         "The draft is an ordinary pipeline stage, so it uses whichever "
         "writing tool you picked."),
        ("check", "You read it before it goes",
         "Nothing is sent until you have seen the subject and the body for "
         "yourself."),
        ("mail", "Sent from your own account",
         "Over SMTP, from the address you set up. Prism has no mail server "
         "of its own."),
    ]

    PLACEHOLDERS = [
        ("send all the clients in the CSV a note convincing them to buy from "
         "us, use brochure.pdf so you know who we are",
         "Attach the CSV and the brochure; the recipients come off the "
         "list, the tone comes off the file."),
    ]
    ...
    def tool_roles(self):
        ...
        brains, content that you have set, which is why Writing wins when it
        is there."""
        agents = self._agents()
        available = [s for s in ("research", "brains", "content")
                     if agents.get(s)]
        return [
            ("pencil", "Drafts the email",
             "Your Writing tool if you have one, otherwise Reasoning, "
             "otherwise Research.",
             agents[available[-1]] if available else ""),
        ]

    def status_block(self):
        """The one thing this add-on genuinely knows about itself: whether a
        sending account has been set up, and which one."""
        account = ((self.cfg.get("email") or {}).get("address") or "").strip()
        wrap = QFrame()
        col = QVBoxLayout(wrap)
        col.setContentsMargins(0, 0, 0, 0)
        col.setSpacing(2)
        col.addWidget(C.kicker(i18n.t("Sending account"), muted=True),
                      alignment=Qt.AlignRight)
        if account:
            line = _Elided(account, "SUPPORT", theme.TEXT, weight=500,
                           grow=False, min_w=120, max_w=260,
                           mode=Qt.ElideMiddle)
        else:
            line = _Elided(i18n.t("Not set up yet"), "SUPPORT", theme.WARN_INK,
                           grow=False, min_w=120, max_w=260)
        line.setAlignment(Qt.AlignRight | Qt.AlignVCenter)
        col.addWidget(line, alignment=Qt.AlignRight)
        return wrap
