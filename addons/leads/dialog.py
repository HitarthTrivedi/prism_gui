...
from addons.leads.workers import (LeadsExportWorker, LeadsSendWorker,
                                  ProspectorWorker, SourceWorker)
from addons.leads.cockpit import LeadsCockpit
...
class LeadsDialog(PrismDialog):
    ...
    def _build_results(self):
        ...
        # The dense "A" cockpit: a multi-select lead table with a filter rail,
        # counter strip, sticky bulk bar and an on-demand dossier + sequence
        # drawer. Bulk actions come back as signals; Export writes the sheets
        # now, and the sequence-on-selection actions land with the Phase-2
        # stop-on-reply engine (until then they point at the real "Send all").
        self._cockpit = LeadsCockpit()
        self._cockpit.exportRequested.connect(lambda _sel: self._export())
        self._cockpit.sequenceRequested.connect(lambda _sel: self._soon("Add to sequence"))
        self._cockpit.verifyRequested.connect(lambda _sel: self._soon("Verify"))
        self._cockpit.saveListRequested.connect(lambda _sel: self._soon("Save to list"))
        self.body.addWidget(self._cockpit, stretch=1)
    ...
    def _on_prepared(self, res, drafts):
        ...
        self._cockpit.set_dossiers(res.dossiers, drafts)
    ...
    def _soon(self, label):
        """Bulk-bar actions whose backend is the Phase-2 stop-on-reply sequence
        engine. Honest placeholder — nothing is faked; it points at the real
        send/export that already work."""
        QMessageBox.information(
            self, i18n.t("Leads & Outreach"),
            i18n.t("'{a}' on the selected leads is the next build step (the "
                   "stop-on-reply sequence engine). For now, use 'Send all' to "
                   "send the reviewed drafts, or 'Export sheets' to save them.")
            .format(a=label))
